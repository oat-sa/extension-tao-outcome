    ...
    extends tao_models_classes_ClassService
{
    ...
    public function getResultStorages(){
        $storageClass = new core_kernel_classes_Class(ResultServerService::PROPERTY_MODEL);
        
        $readableStorages = array();
        $writableStorages = array();
        
        foreach ($storageClass->getInstances() as $storage) {
            $impl = $storage->getUniquePropertyValue(new core_kernel_classes_Property(ResultServerService::PROPERTY_MODEL_IMPL));
    ...
    public function migrateData( $sourceStorages, $targetStorages){
        ...
        foreach ($sourceStorages as $sourceStorage) {
            ...
            $implLiteral = $sourceStorageResource->getUniquePropertyValue(new core_kernel_classes_Property(ResultServerService::PROPERTY_MODEL_IMPL));
        ...
        foreach ($targetStorages as $targetStorage) {
            ...
            $implLiteral = $targetStorageResource->getUniquePropertyValue(new core_kernel_classes_Property(ResultServerService::PROPERTY_MODEL_IMPL));
            $impl = $implLiteral->__toString(); 
            $interfaces = class_implements($impl);
            if (!(in_array('taoResultServer_models_classes_WritableResultStorage', $interfaces))) {
