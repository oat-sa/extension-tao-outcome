...
class taoResultServer_models_classes_ResultServer
{
    ...
    public function __construct($resultServer, $additionalStorages = array())
    {
        ...
        // the static storages
        if ($this->resultServer->getUri() != ResultServerService::RESULT_SERVER) {
            $resultServerModels = $this->resultServer->getPropertyValues(new core_kernel_classes_Property(ResultServerService::MODEL_PROP));
            ...
            foreach ($resultServerModels as $resultServerModelUri) {
                ...
                $this->addImplementation($resultServerModel->getUniquePropertyValue(new core_kernel_classes_Property(ResultServerService::MODEL_IMPL_PROP))->literal);
            }
        }
